...
import lambda_practice.Methodlarim;
...
System.out.println("yemek.stream().noneMatch(t->t.toString().length()<7) = " + yemek.stream().noneMatch(t -> t.toString().length() < 7));
    ...
    public static void karakterEnFazla(List<String> yemek) {
        ...
        //2.yol
        yemek.stream().sorted(Comparator.comparing(t -> t.toString().length()).reversed()).limit(1).collect(Collectors.toList()).forEach(Methodlarim::yazdir);
        // String olarak yazdiirir

        //3.yol
        System.out.println(yemek.stream().sorted(Comparator.comparing(String::length).reversed()).findFirst().get());
        // String olarak yazdirir
